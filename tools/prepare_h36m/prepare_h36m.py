...
def get_mask(subject_dir, img_name):
    msk_path = os.path.join(subject_dir, 'mask_cihp', img_name.replace('.jpg', '.png'))
...
def process_view(subject_dir, img_paths, skip_frame, i, select_view, annots, output_path, scale_ratio=1.0):
...
def main(cfg):
    ...
    dataset_dir = cfg['dataset']['data_path']
    ...
    skip_frame = cfg['dataset']['skip_frame']
    scale_ratio = cfg['dataset']['scale_ratio']
    volume_size = cfg['dataset']['volume_size']
    
    # obtain directories
    subject_dir = os.path.join(dataset_dir, f"{subject}", "Posing")
    ...
    # process views
    for i, view in enumerate(select_views):
        img_paths = np.array([
            np.array(multi_view_paths['ims'])[i]
            for multi_view_paths in img_path_frames_views
        ])
        img_paths = img_paths[start_frame:end_frame:skip_frame]
        
        output_path_view = os.path.join(output_path, str(view))
        os.makedirs(output_path_view, exist_ok=True)
        process_view(subject_dir, img_paths, skip_frame, i, view, annots, output_path_view, )
    ...
    for idx, ipath in enumerate(tqdm(img_paths)):
        ...
        smpl_params = np.load(
            os.path.join(smpl_params_dir, f"{idx}.npy"),
...
if __name__ == '__main__':
    '''python tools/prepare_h36m/prepare_h36m.py --conf tools/prepare_h36m/confs_preprocess/S6.yaml'''
    cfg = parse_cfg()
    main(cfg)