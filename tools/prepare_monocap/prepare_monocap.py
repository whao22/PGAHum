...
def get_mask(subject_dir, img_name):
    msk_path = os.path.join(subject_dir, img_name.replace('images', 'mask'))
...
def main(cfg):
    ...
    for idx, ipath in enumerate(tqdm(img_paths)):
        ...
        smpl_params = np.load(
            os.path.join(smpl_params_dir, f"{smpl_idx+1}.npy"),
...
if __name__ == '__main__':
    '''python tools/prepare_monocap/prepare_monocap.py --conf tools/prepare_monocap/confs_preprocess/vlad.yaml'''
    cfg = parse_cfg()
    main(cfg)