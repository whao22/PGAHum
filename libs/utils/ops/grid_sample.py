...
from libs.utils.ops import custom_ops
import os

grid_sample_grad = None

def _init():
    global grid_sample_grad
    if grid_sample_grad is None:
        grid_sample_grad = custom_ops.get_plugin(
            module_name='grid_sample_grad',
...
class _GridSample2dBackward(torch.autograd.Function):
    ...
    def backward(ctx, grad2_grad_input, grad2_grad_grid):
        ...
        out = grid_sample_grad.grad2_2d(grad2_grad_input, grad2_grad_grid, grad_output,
...
class _GridSample3dBackward(torch.autograd.Function):
    ...
    def backward(ctx, grad2_grad_input, grad2_grad_grid):
        grad_output, input, grid = ctx.saved_tensors
        assert grad_output.is_cuda and input.is_cuda and grid.is_cuda and grad2_grad_input.is_cuda and grad2_grad_grid.is_cuda
        out = grid_sample_grad.grad2_3d(grad2_grad_input, grad2_grad_grid, grad_output,
                                        input, grid, ctx.padding_mode, ctx.align_corners)
