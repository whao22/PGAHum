...
std::vector<at::Tensor> Fast4x4Minv(at::Tensor ms)
{
	CHECK_INPUT(ms);
	...
	auto options_bool_nograd = torch::TensorOptions()
	                            ...
	                            .device(ms.device())
	                            .requires_grad(false);
	auto invs=torch::zeros({N,4,4},options_float_nograd);
	auto checks=torch::zeros({N},options_bool_nograd);
	if(ms.type().scalarType()==at::ScalarType::Float)
		M4x4Inv_float(ms.data<float>(), invs.data<float>(), checks.data<bool>(),N);
	else
		M4x4Inv_double(ms.data<double>(), invs.data<double>(), checks.data<bool>(),N);
	return {invs,checks};

}

at::Tensor Fast4x4Minv_backward(at::Tensor grads, at::Tensor invs)
{
	CHECK_INPUT(grads);
	CHECK_INPUT(invs);
	...
	auto options_float_nograd = torch::TensorOptions()
	                            ...
	                            .device(invs.device())
	                            .requires_grad(false);
	auto outs=torch::zeros({N,4,4},options_float_nograd);
	if(invs.type().scalarType()==at::ScalarType::Float)
		M4x4Inv_backward_float(grads.data<float>(), invs.data<float>(), outs.data<float>(),N);
	else
		M4x4Inv_backward_double(grads.data<double>(), invs.data<double>(), outs.data<double>(),N);
	return outs;

}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("Fast4x4Minv", &Fast4x4Minv, "fast batch 4x4 matrix inversion (CUDA)");
  m.def("Fast4x4Minv_backward", &Fast4x4Minv_backward, "fast batch 4x4 matrix inversion backward (CUDA)");
}