...
class ZJUMoCapDataset(torch.utils.data.Dataset):
    ...
    #     else:
    #         hit_mask_lst=[]
    #         # the odd-th intersection is the indent point and the even-th intersection is the exit point.
    #         # for simplicity, we 
    #         for i in range(hit_masks.shape[-1]//2):
    #             hit_mask = hit_masks[..., i*2][..., None] * hit_masks[..., i*2+1][..., None]
    #             hit_mask_lst.append(hit_mask)
    #             hit_mask_lst.append(hit_mask)
    #         hit_masks = np.concatenate(hit_mask_lst, axis=-1)
        
    #     # calculate the distance of intersections and camera location, where the max 
    #     # number of intersection is dist[..., 1:-1].shape(-1)
    #     dists = np.concatenate(dist_lst, axis=-1)
    #     dists[dists==np.inf] = 0
    #     hit_dists = np.cumsum(dists[..., 1:-1], axis=-1)
    ...
    #     if N_interval == 0:
    #         z_vals = np.linspace(0, 1, N_samples)
    #         # z_vals = np.tile(z_vals[None, :], [N_rays, 1])
    ...
    #         j = 2*i
    #         k = 2*i + 1
    #         cur_near = hit_dists[..., j:j+1]
    #         cur_far = hit_dists[..., k:k+1]
    #         cur_interval = cur_far - cur_near
    #         interval_lst.append(cur_interval)
    #     interval_lst = np.concatenate(interval_lst, axis=-1)
    ...
    #     # construct z_vals
    #     N_points_per_interval = N_samples//N_interval
    #     interval_mask = np.arange(N_samples)
    ...
    #         if False:
    #             # aggsign the z_vals of non-intersectd rays as [cur_near.min(), cur_far.max()]
    #             if i == N_interval - 1 :
    #                 column_mask = interval_mask[i*N_points_per_interval: ]
    #             else:
    #                 column_mask = interval_mask[i*N_points_per_interval: (i+1)*N_points_per_interval]
    #             t = np.linspace(0, 1, len(column_mask))
                
    #             row_mask = hit_masks[:, 2*i]
    #             cur_near_new = np.tile(cur_near[row_mask].min(), cur_near[~inter_mask].shape)
    #             cur_far_new = np.tile(cur_far[row_mask].max(), cur_far[~inter_mask].shape)
    #             mask1 = np.zeros_like(z_vals)
    #             mask2 = np.zeros_like(z_vals)
    #             mask1[~inter_mask] = 1
    #             mask2[:,column_mask] = 1
    #             z_vals[(mask1 * mask2).astype(np.bool_)] = (cur_near_new[:, None] + (cur_far_new - cur_near_new)[:, None] * t[None, :]).reshape(-1)
